...
### Phase 6.5 (root-cause fixes uncovered by pre-release QA + a user tray crash)

- **Root cause 1 -- queue lock thread affinity.** `src/queue.ps1` swapped `System.Threading.Mutex` for `System.Threading.SemaphoreSlim` on both the queue and recent locks. Under Mutex a `WaitOne` on the dispatcher thread could hand off to a threadpool `Release`, which threw `Object synchronization method was called from an unsynchronized block of code` and killed the tick timer (observed live at 2026-09-03 18:27:48). Semaphores are thread-agnostic; we lose named cross-process locking, but the singleton in `grab-app.ps1` already guarantees a single writer. Added thread-local re-entrancy tracking so mutating helpers (Set-JobStatus / Cancel-QueueJob / etc.) can still call `Read-Queue` and `Write-Queue` from inside a `_WithQueueMutex` block without deadlocking on SemaphoreSlim's non-reentrant Wait. Regression test hammers the lock from six threadpool tasks in parallel.
- **Root cause 2 -- Desktop shortcut invisibility under OneDrive KFM.** New `Get-VisibleDesktopPath` returns whatever `[Environment]::GetFolderPath('Desktop')` reports (OneDrive-redirected or not). `install.ps1` and `Invoke-SelfHealSweep` now use it for Desktop shortcut creation so KFM users actually see the icon. The old `Get-LocalDesktopPath` stays for uninstall-side backward-compat sweeping of stale pre-6.5 installs; `Get-LocalStartupPath` still refuses OneDrive (belt-and-braces backup to HKCU\Run).
- **Root cause 3 -- Start Menu shortcut missing from dev-clone installs.** New `Set-StartMenuShortcut([bool])` centralises the per-user Start Menu entry at `%APPDATA%\Microsoft\Windows\Start Menu\Programs\GRAB\GRAB.lnk`. `install.ps1` calls it, `Invoke-SelfHealSweep` recreates it if missing, `uninstall.ps1` sweeps the shortcut + its containing folder. Test-hook env-var `GRAB_STARTMENU_OVERRIDE` mirrors the Desktop / Startup overrides.
- **QA-1** -- winget installer.yaml sha256 test now accepts either the pre-build placeholder or a real 64-hex hash, and when a hash is present it must match `dist/SHA256SUMS.txt` for `GRAB-Setup.exe` (catches drift between the manifest and the actual build).
- **QA-2** -- `portable-mode.flag` now actually redirects `$script:AppData` to `<repo>/grab-data` when placed next to `grab-app.vbs` (previously documented in `PORTABLE.txt` but never wired up).
- **QA-3** -- `Send-Toast` now falls back to `$script:Tray.ShowBalloonTip` when BurntToast isn't installed (the installer + portable zip both skip the Python + BurntToast install), so installer users get real toasts instead of silently dropped notifications.
- **QA-4** -- removed the broken `{group}\GRAB downloads folder` Inno Setup shortcut that pointed at `assets\icon.ico` instead of a launcher. The tray menu's "Open downloads" item already covers this at click time from the live `downloadFolder` config.
- **Portable zip size** -- `build-installer.ps1` now packs with `7z.exe a -tzip -mx9 -m0=LZMA2` when 7-Zip is available (it always is because Get-Ffmpeg installs it); shaves the portable zip by ~30% vs `Compress-Archive`. Falls back to `Compress-Archive` when 7z is missing.
- Docs: `docs/file-map.md` updated to remove the stale `assets/icon-32.png` row and add `docs/smartscreen.md`; README + `docs/smartscreen.md` reference the versioned `GRAB-Portable-v0.3.0.zip` filename.
